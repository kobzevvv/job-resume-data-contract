...
/**
 * Debug ResumeData Missing - Focused Test
 * Test with data-engineer.txt to verify resumeData is properly stored
 */
...
console.log('🔍 Debug ResumeData Missing - Focused Test');
console.log('==========================================\n');
...
function loadDataEngineerResume() {
  try {
    ...
    lines.forEach(line => {
      if (
        line.includes('2021-08') ||
        line.includes('2019-01') ||
        line.includes('2018-06')
      ) {
...
async function debugResumeDataMissing() {
  try {
    ...
    if (responseData.success && responseData.data) {
      ...
      // Show the exact data that should be logged
      console.log('\n📋 Sample Data That Should Be Logged:');
      console.log('-----------------------------------');
      console.log('resumeData: {');
      if (responseData.data.desired_titles) {
        console.log(
          `  desired_titles: ${JSON.stringify(responseData.data.desired_titles)}`
        );
      }
      if (
        responseData.data.experience &&
        ...
        console.log(
          `  experience: [${responseData.data.experience.length} entries]`
        );
        console.log(
          `    First: ${responseData.data.experience[0].title} at ${responseData.data.experience[0].employer}`
        );
      }
      if (responseData.data.skills) {
        console.log(`  skills: [${responseData.data.skills.length} skills]`);
      }
      console.log('}');
    } else {

...
async function testHealthEndpoint() {
  ...
  try {
    ...
    logTest('Health endpoint returns healthy status', data.status === 'healthy');
    logTest('Health endpoint includes version', !!data.version);
    logTest('Health endpoint includes endpoints array', Array.isArray(data.endpoints));
    logTest('Health endpoint includes AI status', ['available', 'unavailable'].includes(data.ai_status));
...
async function processResume(resumeFile, resumeText) {
  ...
  try {
    ...
    logTest(`${resumeFile}: Response has correct structure`, 
      typeof data === 'object' && 
    ...
    logTest(`${resumeFile}: Processing time reasonable`, 
    ...
    if (data.success && data.data) {
      ...
      const requiredFields = ['desired_titles', 'summary', 'skills', 'experience'];
      for (const field of requiredFields) {
        logTest(`${resumeFile}: Has ${field}`, 
          data.data[field] !== undefined,
          data.data[field] ? `Found: ${Array.isArray(data.data[field]) ? data.data[field].length + ' items' : 'present'}` : 'Missing'
        );
      }
      
      // Validate data types
      logTest(`${resumeFile}: desired_titles is array`, Array.isArray(data.data.desired_titles));
      logTest(`${resumeFile}: summary is string`, typeof data.data.summary === 'string');
      logTest(`${resumeFile}: skills is array`, Array.isArray(data.data.skills));
      logTest(`${resumeFile}: experience is array`, Array.isArray(data.data.experience));
      
      // Content validation
      if (data.data.desired_titles && data.data.desired_titles.length > 0) {
        logTest(`${resumeFile}: Has job titles`, true, `Found: ${data.data.desired_titles.join(', ')}`);
      }
      ...
      if (data.data.skills && data.data.skills.length > 0) {
        ...
        logTest(`${resumeFile}: Has skills`, true, `Found ${skillNames.length} skills: ${skillNames.slice(0, 3).join(', ')}${skillNames.length > 3 ? '...' : ''}`);
      }
      
      if (data.data.experience && data.data.experience.length > 0) {
        logTest(`${resumeFile}: Has work experience`, true, `Found ${data.data.experience.length} positions`);
      }
      
    } else {
      logTest(`${resumeFile}: Processing failed`, false, 
    ...
    console.log(`   ⏱️  Processing time: ${data.processing_time_ms}ms (Network + Processing: ${processingTime}ms)`);
    console.log('');
    
  } catch (error) {
    logTest(`${resumeFile}: API call successful`, false, `Error: ${error.message}`);
  }
}
...
async function runTests() {
  ...
  try {
    const resumeFiles = readdirSync(SAMPLE_RESUMES_DIR).filter(file => file.endsWith('.txt'));

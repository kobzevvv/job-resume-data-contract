import { Env, ProcessResumeRequest, ProcessResumeResponse, HealthResponse, ResumeData } from './types';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      const clientId = request.headers.get('cf-ray') || request.headers.get('cf-connecting-ip') || 'unknown';
 ...
 */
async function handleProcessResume(request: Request, env: Env): Promise<Response> {
  const getElapsed = measureTime();
  ...
  try {
    ...
    if (!validateContentType(request)) {
      return createErrorResponse(400, 'INVALID_CONTENT_TYPE', 'Content-Type must be application/json');
    }
    
    // Validate request size
    if (!validateRequestSize(request)) {
      return createErrorResponse(413, 'PAYLOAD_TOO_LARGE', 'Request body too large (max 50KB)');
    }
    ...
    try {
      ...
      return createErrorResponse(400, 'INVALID_JSON', error instanceof Error ? error.message : 'Invalid JSON');
    }
    
    // Validate required fields
    if (!requestData.resume_text || typeof requestData.resume_text !== 'string') {
      return createErrorResponse(400, 'MISSING_RESUME_TEXT', 'resume_text is required and must be a string');
    }
    
    if (requestData.resume_text.trim().length < 50) {
      return createErrorResponse(400, 'RESUME_TEXT_TOO_SHORT', 'resume_text must be at least 50 characters');
    }
    ...
    if (validation.warnings.length > 0) {
      logEvent('warn', 'Validation warnings', { warnings: validation.warnings });
    }
    ...
    const response: ProcessResumeResponse = {
      ...
      unmapped_fields: requestData.options?.include_unmapped !== false ? aiResult.unmapped_fields : [],
